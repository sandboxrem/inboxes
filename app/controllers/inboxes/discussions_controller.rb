    ...
    unless @discussion.save
      respond_to do |format|
        format.html { 
          ...
          :notice => t("inboxes.discussions.started")
        }
        format.js
      end
    else
    ...
    if @discussion.recipient_ids && @discussion.recipient_ids.size == 1 &&
      ...
      if discussion
        ...
        @discussion.messages.each do |message|
          @message = Message.create(:discussion => discussion, :user => current_user, 
        ...
        respond_to do |format|
          format.html {
            redirect_to inboxes.discussion_url(discussion), 
             :notice => t("inboxes.discussions.already_exists", 
              :user => user[Inboxes::config.user_name]) 
          }
          format.js
        end 
      end
